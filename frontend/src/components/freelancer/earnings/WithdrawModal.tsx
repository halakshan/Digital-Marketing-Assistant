...
import { METHODS } from "./earningsData";
...
export default function WithdrawModal({ availableBalance, onClose, onConfirm }: Props) {
    ...
    <>
      <style>{`
        ...
        * { box-sizing:border-box; }
        ...
        .confirm-btn:hover { background:#6d28d9 !important; transform:translateY(-1px); }
        ...
        >
          ...
          {step === "form" && (
            <>
                ...
                <div style={{ display: "flex", flexDirection: "column", gap: 8 }}>
                  {METHODS.map(m => (
                    ...
                    >
                      ...
                      {m.primary && (
                        <span style={{ fontSize: 10, padding: "2px 8px", borderRadius: 20, background: C.purpleGlow, border: `1px solid rgba(124,58,237,0.3)`, color: C.accent, fontWeight: 600 }}>Primary</span>
                      )}
                      <div style={{ width: 18, height: 18, borderRadius: "50%", border: `2px solid ${selectedMethod === m.name ? C.purple : C.border}`, display: "flex", alignItems: "center", justifyContent: "center", flexShrink: 0 }}>
                        {selectedMethod === m.name && <div style={{ width: 10, height: 10, borderRadius: "50%", background: C.purple }} />}
                      </div>
                    </div>
                  ))}
                </div>
              ...
              <div style={{ display: "flex", gap: 10 }}>
                <button className="confirm-btn" onClick={handleContinue} style={{ flex: 1, background: C.purple, color: "#fff", border: "none", padding: "12px", borderRadius: 10, fontSize: 14, fontWeight: 600, cursor: "pointer", fontFamily: "'Sora',sans-serif", transition: "all 0.18s" }}>
                  Continue →
                </button>
          ...
          {step === "confirm" && (
            <>
              ...
              <div style={{ display: "flex", gap: 10 }}>
                <button className="confirm-btn" onClick={handleFinalConfirm} style={{ flex: 1, background: C.purple, color: "#fff", border: "none", padding: "12px", borderRadius: 10, fontSize: 14, fontWeight: 600, cursor: "pointer", fontFamily: "'Sora',sans-serif", transition: "all 0.18s" }}>
                  ✅ Confirm Withdraw
                </button>
          ...
          {step === "success" && (
            <div style={{ textAlign: "center", padding: "20px 0" }}>
              ...
              <div style={{ marginTop: 16, padding: "10px 16px", background: "rgba(34,197,94,0.08)", border: "1px solid rgba(34,197,94,0.25)", borderRadius: 10 }}>
